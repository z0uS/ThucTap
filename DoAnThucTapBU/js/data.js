...
async function getById(key, id) {
  if (key === DB_KEYS.faculty) {
    return await fetchAPI(`/admin/lecturers/${id}`);
  }
  // Fallback
  return (await getAll(key)).find(x => x.id == id) || null;
}
...
async function adminLogin(username, password) {
  try {
    ...
    if (res.ok) {
      ...
      dbSet(DB_KEYS.session, { username: data.username, basicAuth: token, loginTime: Date.now() });
      return true;
    }
