...
async function renderDashboardStats() {
  try {
    ...
    setEl('stat-posts', stats.totalPosts);
    setEl('stat-faculty', stats.totalFaculty);
    setEl('stat-categories', stats.totalCategories);
    setEl('stat-announcements', stats.totalAnnouncements);
    setEl('stat-views', stats.totalViews.toLocaleString('vi-VN'));
  } catch (e) {
...
async function renderBarChart(containerId) {
  ...
  try {
    const data = await fetchAPI('/admin/dashboard/chart');
    const labels = data.labels; // Array of MM/yyyy
    const postCounts = data.postCounts;
    const maxCount = Math.max(...postCounts, 1);

    container.innerHTML = labels.map((label, i) => `
      <div class="bar-item">
        <div class="bar-value">${postCounts[i]}</div>
        <div class="bar" style="height:${(postCounts[i] / maxCount) * 120}px" title="${postCounts[i]} bài"></div>
...
async function saveFaculty() {
  ...
  const data = {
    ...
    fullName: nameVal,
    degree: degreeVal,
    position: positionVal || 'Giảng viên',
    specialization: document.getElementById('faculty-specialization').value.trim(),
    email: emailVal || 'no-email@tlu.edu.vn',
    avatar: document.getElementById('faculty-image').value.trim(),
    image: document.getElementById('faculty-image').value.trim(),
    adminId: adminId // Thêm adminId nếu backend cần (dù LecturerRequest hiện tại chưa bắt buộc)
  };
...
async function saveAnnc() {
  ...
  const data = {
    ...
    adminId: adminId // Bắt buộc bởi AnnouncementRequest
  };
