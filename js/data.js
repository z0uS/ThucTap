...
async function getAll(key) {
  try {
    if (key === DB_KEYS.lecturers || key === DB_KEYS.faculty) {
      return await fetchAPI('/public/lecturers');
    }
    if (key === DB_KEYS.categories) {
      return await fetchAPI('/public/categories');
    }
    if (key === DB_KEYS.posts) {
      const posts = await fetchAPI('/public/posts');
      // Đồng bộ dữ liệu từ backend sang định dạng frontend mong đợi
      return posts.map(p => ({
        ...p,
        image: p.thumbnail || '',
        date: p.createdAt ? p.createdAt.split('T')[0] : '',
        author: p.author ? p.author.fullName : 'Admin',
        category: p.category ? p.category.id : '',
        categoryName: p.category ? p.category.name : '',
        views: p.viewCount || 0
      }));
    }
    if (key === DB_KEYS.announcements) {
      return await fetchAPI('/public/announcements');
    }
  } catch (e) {
    console.warn("API GET failed for", key, e);
  }
  // Fallback to local
  return dbGet(key) || [];
}
...
async function getById(key, id) {
  try {
    return (await getAll(key)).find(x => String(x.id) === String(id)) || null;
  } catch (e) {
    console.error("Lỗi getById:", e);
    return null;
  }
}
...
async function create(key, item) {
  try {
    if (key === DB_KEYS.faculty) {
      return await fetchAPI('/admin/lecturers', 'POST', item);
    }
    if (key === DB_KEYS.categories) {
      return await fetchAPI('/admin/categories', 'POST', item);
    }
    if (key === DB_KEYS.posts) {
      return await fetchAPI('/admin/posts', 'POST', item);
    }
    if (key === DB_KEYS.announcements) {
      return await fetchAPI('/admin/announcements', 'POST', item);
    }
...
async function update(key, id, data) {
  try {
    if (key === DB_KEYS.faculty) {
      return await fetchAPI(`/admin/lecturers/${id}`, 'PUT', data);
    }
    if (key === DB_KEYS.categories) {
      return await fetchAPI(`/admin/categories/${id}`, 'PUT', data);
    }
    if (key === DB_KEYS.posts) {
      return await fetchAPI(`/admin/posts/${id}`, 'PUT', data);
    }
    if (key === DB_KEYS.announcements) {
      return await fetchAPI(`/admin/announcements/${id}`, 'PUT', data);
    }
    // Posts ko có PUT, fallback
...
async function remove(key, id) {
  try {
    if (key === DB_KEYS.faculty) {
      await fetchAPI(`/admin/lecturers/${id}`, 'DELETE');
      return;
    }
    if (key === DB_KEYS.categories) {
      await fetchAPI(`/admin/categories/${id}`, 'DELETE');
      return;
    }
    if (key === DB_KEYS.posts) {
      await fetchAPI(`/admin/posts/${id}`, 'DELETE');
      return;
    }
    if (key === DB_KEYS.announcements) {
      await fetchAPI(`/admin/announcements/${id}`, 'DELETE');
      return;
    }
  } catch (e) { console.error("Remove error", e); throw e; }

  // Fallback
  let list = await getAll(key);
...
async function trackView(postId) {
  try {
    // 1. Gọi API tăng view thật trên server
    await fetchAPI(`/public/posts/${postId}/view`, 'POST');
    return true;
  } catch (e) {
    console.warn("View increment failed, using local fallback", e);
...
async function adminLogin(username, password) {
  try {
    ...
    if (res.ok) {
      ...
      // Lưu data vào trường user để session.user.id hoạt động
      dbSet(DB_KEYS.session, { user: data, username: data.username, basicAuth: token, loginTime: Date.now() });
      return true;
    }
  } catch (e) { console.error(e); }

  // Fallback dev mode if backend is down
  if (username === 'admin' && password === 'admin123') {
    dbSet(DB_KEYS.session, { user: { id: 'ADMIN_DEV', username: 'admin' }, username, loginTime: Date.now() });
    return true;
  }
  return false;
}
...
async function searchAll(query) {
  ...
  const [posts, anns, faculty] = await Promise.all([
    getAll(DB_KEYS.posts),
    getAll(DB_KEYS.announcements),
    getAll(DB_KEYS.faculty)
  ]);
